...
export async function responseMessage(DataContent) {
	// * Generate text should be in server side so that it can read the API key

	const response = await generateText({
		model: openai("gpt-4o"),
		system: "盡你所能分析學生在白板上寫的數學推導，學生正在計算$ U = -ax^2 + bx^4 $ 的最小值在哪裡，為多少?，學生常使用dx/dt 做為微分符號，或是 /dot{x} 表示。首先摘要學生在白板上繪製的所有內容，再來評論他的推導是否正確，提供改進的建議。 answer in 繁體中文，保持幽默，使用 Emoji",
		messages: [{
			role: "user",
			content: [
				{
					type: "text",
					text: "以下圖片是我在白板上寫的數學推導，請勿直接要跟我講答案或計算，請分析我的圖片，如果是空白的是不正常的，請仔細描述圖片上的內容",
				},
				{
					type: "image",
					image: DataContent
