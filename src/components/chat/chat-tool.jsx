...
import emotionUrls from "@/app/(lab)/csr-chat/(tool-call)/sticker-sender/emotionUrls"
...
export function ToolCall({ toolName, args }) {
	switch (toolName) {
		...
		case 'sticker': {
			return (
				<>
					<img src={emotionUrls.find((item) => item.emotion === args.mood).url} />
					{console.log(emotionUrls.find((item) => item.emotion === args.mood).url)}
				</>
			)
		}
		default: {
